...
export class FigmaScreenComponent implements OnInit, OnDestroy {
  ...
  // The HTML content pulled from assets/spotify-1-0-3.html stripped of the outer html/head/body,
  // preserving exact inner structure and asset paths (assets/...).
  // PUBLIC_INTERFACE
  htmlContent = `
  ...
  // Use any here to avoid linter complaining about DOM lib types in Node context.
  ...
  ngOnInit(): void {
    ...
    // Inject stylesheets from assets. Use absolute /assets paths relative to app base href (/).
    ...
    // Initialize shared and screen-specific JS after DOM paints.
  ...
  ngOnDestroy(): void {
    ...
    // Cleanup dynamically added tags to avoid duplication on navigation.
